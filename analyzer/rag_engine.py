...
📌 최근 인기 키워드(신메뉴·재료·콘셉트)를 활용해 각 전략 1에 메뉴 개발 /콘셉트 아이디어와 홍보 문구를 1개 포함하세요.
...
- 각 전략의 제목 또는 문구·실행 방법 안에 위 키워드 목록에서 최소 2개 이상을 정확히 포함하세요
""",
...
📌 최근 인기 키워드(신메뉴·재료·콘셉트)를 각 기간별 전략 최소1개 반드시 반영해 신메뉴 개발/프로모션 아이디어를 구체적으로 제시하세요.예를 들어
'현재 인기 메뉴는 곱창이니 이러한 신메뉴를 개발하는 방법도 고려해보세요'
{combined_context}
...
- 각 기간별 전략에는 위 키워드 목록에서 최소 2개 이상을 그대로 포함하고, 해당 키워드를 활용한 메뉴/프로모션 아이디어를 명시하세요
""",
...
📌 최신 인기 키워드(신메뉴·재료·콘셉트)를 반영해 각 개선 전략에 메뉴 업그레이드/홍보 아이디어를 명시하세요.예를 들어 '현재 인기 메뉴는 곱창이니 이러한 신메뉴를 개발하는 방법도 고려해보세요'
...
- 각 개선 전략에는 위 키워드 목록에서 최소 2개 이상을 정확히 언급하고, 이를 반영한 메뉴/홍보 개선안을 제시하세요
""",
    }
...
def generate_rag_summary(
    ...
    keyword_top10: Optional[List[Dict[str, Any]]] = None,
) -> Dict[str, Any]:
    t_start = time.time()
    ...
    try:
        ...
        # ✅ v2/v3 모드일 때 업종 정보를 컨텍스트에 주입
        if store_type and mode in {"v2", "v3"}:
            industry_guidance = {
                "v2": (
                    "재방문율을 높일 수 있는 멤버십, 이벤트, 고객 경험 개선 아이디어를 "
                    "이 업종 특성에 맞춰 제시하세요."
                ),
                "v3": (
                    "이 업종의 전형적인 영업 방식과 고객 행동 패턴을 고려해 "
                    "문제를 진단하고 개선 전략을 제시하세요."
                ),
            }
            store_label = str(store_type)
            combined_context += (
                f"🏪 [매장 업종 정보]\n"
                f"현재 매장은 '{store_label}' 업종에 해당합니다.\n"
                f"{industry_guidance[mode]}\n\n"
            )
        if keyword_top10:
            keyword_lines = []
            keyword_names = []
            for item in keyword_top10:
                if isinstance(item, dict):
                    name = item.get("keyword") or item.get("키워드") or "-"
                    ratio = item.get("평균검색비율") or item.get("value") or item.get("ratio")
                    ...
                    line = f"- {name}"
                    if ratio_text:
                        line += f" (인기도 {ratio_text})"
                    keyword_lines.append(line)
                    if name:
                        keyword_names.append(name)
                else:
                    text_item = str(item)
                    keyword_lines.append(f"- {text_item}")
                    keyword_names.append(text_item)
            combined_context += (
                "🍽️ [최근 인기 메뉴·키워드]\n"
                + "\n".join(keyword_lines[:10])
                + "\n\n"
                + "👉 위 키워드(신메뉴/콘셉트/재료)를 활용해 제안 전략과 홍보 문구에 최신 트렌드를 반드시 반영하세요.\n"
                + "👉 각 전략에는 반드시 아래 키워드 중 최소 2개 이상을 그대로 포함해야 합니다:\n"
                + ", ".join(keyword_names[:10])
                + "\n\n"
            )
        combined_context += (
