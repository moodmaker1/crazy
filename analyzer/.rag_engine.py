...
def generate_rag_summary(mct_id: str, mode: str = "v1", top_k: int = 5) -> Dict[str, Any]:
    ...
    try:
        ...
        이 데이터를 기반으로 **AI 마케팅 리포트를 작성**하세요.

        {combined_context}

        작성 지침:
        1️⃣ 매장 핵심 요약 — 현재 상태를 1문단으로 설명  
        2️⃣ 데이터 기반 인사이트 — 최소 3개, 각 인사이트마다 **근거 문장(출처 데이터)** 을 괄호 안에 명시  
            예: "10~20대 여성 고객 비중이 높음 (출처: 챔스*** 리포트, +3.7pp)"
        3️⃣ 주요 문제점 및 원인 — 원인을 **정량 데이터**와 연결해서 설명  
        4️⃣ 개선 전략 — 단기/중기/장기별로 **구체적 실행안** 제시 (각 전략의 **기대 효과** 포함)  
        5️⃣ 결론 및 우선순위 요약 — 어떤 전략이 가장 시급한지 한 문단으로 명시

        ⚙️ 작성 규칙:
        - 모든 주장에는 반드시 데이터 근거를 명시하세요.
        - 데이터는 [매장 분석 데이터] 또는 [마케팅 전략 데이터]에서 발췌한 내용이어야 합니다.
        - 단순 요약이 아니라, **데이터 기반의 논리적 분석 리포트**로 작성하세요.
        - 결과는 한국어로 작성하세요.
        """
